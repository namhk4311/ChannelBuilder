...
def _chat(system: str, user: str, temperature: float = 0.85, max_tokens: int = 2000) -> str:
    ...
    text = "".join(chunks)
    if not text.strip():
        raise RuntimeError("Stream trả về rỗng")
    return text
...
def generate_script(idea, insight_digest=None, target_duration_sec=48):
    """Từ 1 ý tưởng → script_package: text hook + kịch bản 40-55s + shot list theo
    clip_tag + caption/hashtag. Kho clip đã nằm sẵn trong system prompt."""
    try:
        user_parts = [
