...
2. MỖI MỤC: 1-2 câu ngắn, đánh số rõ ("Thứ nhất...", "Thứ hai...") — xen [normal]/[excited].
...
PRESETS = {
    ...
    "listicle": {
        ...
        "structure_rule": "Cảnh ĐẦU = hook + tổng số mục. MỖI CẢNH GIỮA = 1 mục đánh số (Điều 1, Điều 2, …) — "
                          "event_title nên mở đầu bằng số/thứ tự. CUỐI = chốt. Mỗi cảnh nêu 4-6 dữ kiện nổi bật (số/%/tên riêng).",
        "mood_options": "clean|modern|friendly",
        "image_style": "ảnh nền 9:16 hiện đại, sạch, tối giản, mood {mood}",
        "voiceover_system": _VO_LISTICLE,
        "vo_arc": "Mở [excited] hook + nêu sẽ có N điều → mỗi điều [normal]/[excited] ngắn gọn, đánh số → [excited] chốt.",
...
def len_guide(preset_key: str, n: int) -> str:
    ...
    est_lo, est_hi = n * 4, n * 7
    return (f"{n} cảnh → khoảng {n}-{n + 1} cụm thoại (~{est_lo}-{est_hi} giây), "
            f"mỗi cảnh ~1 ý/cụm. Nhịp: {arc}")
